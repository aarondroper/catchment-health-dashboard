#!/usr/bin/env python3
"""Run deterministic local checks before any public dashboard release.

This check deliberately does not assert that unresolved source terms permit
redistribution. It verifies that a prepared local asset is identifiable,
freshness metadata is present, attribution is in the application, and raw or
generated observation outputs are not tracked. Public release remains blocked
while the runtime contract carries the local-processing release-gate status.
"""
...
ATTRIBUTION = "This work uses data sourced from Environment Canterbury."
...
def check_local_readiness(asset_path: Path, *, max_age_days: int) -> tuple[list[str], bool]:
    ...
    messages.extend([
        ...
        "PUBLIC_RELEASE=blocked (sourceTermsStatus is local_processing_only_release_gate)",
    ])
...
def main() -> int:
    ...
    try:
        messages, _ = check_local_readiness(asset_path, max_age_days=args.max_age_days)
    ...
    if args.require_public_release:
        print("RELEASE_READINESS=failed: public source-term approval is not recorded", file=sys.stderr)
        return 1
    print("RELEASE_READINESS=local-safe-but-not-public")
    return 0
