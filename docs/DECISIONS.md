...
- **Status:** Superseded as the primary dashboard policy by D-018; retained as the strict sensitivity rule
- **Decision:** Preserve raw quality-field representation as `missing_field`, `blank_field`, or `nonempty_code`. Keep missing and blank quality excluded from strict primary eligibility as `missing_quality_field`; keep unfamiliar nonempty codes as `unresolved_quality`; retain documented fair/good records and exclude documented compromised/poor records. The former unflagged inclusion was evaluated only as a diagnostic scenario at this decision point; D-018 records the later adopted exploratory policy.
...
- **Alternatives considered:** Treat omitted/blank quality as usable; reject all records lacking QC500/QC600; implement a source-specific mapping without documentation. The first would materially increase eligible observations but would turn an undocumented absence into an analytical assertion; the latter two would unnecessarily discard documented raw observations. The conservative distinction was adopted for the strict sensitivity mode; D-018 later approved a carefully labelled exploratory inclusion for missing fields.
- **Consequences:** The strict sensitivity build contains 846 eligible observations, 208 usable summaries, and 0 determinate trends. D-018 adopts the separately labelled `published_unflagged` policy for local exploratory dashboard assets while preserving this conservative result for sensitivity comparison.

## D-018 — Adopt `published_unflagged` for the exploratory dashboard

- **Status:** Accepted by owner on 2026-09-13; local dashboard policy
- **Decision:** Use a two-tier quality policy. The primary local dashboard policy includes QC600 as `retained_good_quality`, QC500 as `retained_fair_quality`, and observations returned by the published ECan service with no quality field as `published_unflagged`. A missing field is not called explicitly verified, QC600, or good. Blank fields remain a separate `blank_quality_field` disposition and are excluded because no blank-field semantics were observed or documented. QC400, unknown nonempty codes, invalid values/units, and duplicate conflicts remain excluded from analytical summaries and trends but retained in provenance and counts. The prior strict policy remains available as a processing sensitivity mode.
- **Evidence:** The refreshed profile contains 10,426 observations: 9,434 missing quality fields, 774 QC600, 72 QC500, and 146 QC400. Strict eligibility retained only 846 rows and produced no determinate trends. The source-supported diagnostic comparison included 10,280 rows, produced 1,296 usable summaries, and supported 17 determinate primary trends. The LAWA/Hilltop implementation guidance treats data without a quality code as a distinct published category; it does not establish that these rows are explicitly quality-verified.
- **Alternatives considered:** Keep strict eligibility as the product policy; classify missing quality as explicitly good; or include unknown/blank/poor values. Strict removes approximately 90.5% of returned observations and prevents a useful exploratory view. Calling omission “good” overstates the evidence. Including documented poor or unresolved values would weaken analytical integrity. The adopted policy preserves the middle ground and labels the limitation directly.
- **Consequences:** Local analytical assets use `published_unflagged` and expose the strict comparison in the viability report. The UI emphasizes observed history, distributions, coverage, and quality/provenance context; trends are shown only when minimum-data and censoring rules support them. Public observation redistribution remains blocked by the unresolved Hilltop/source-terms release gate.

## D-019 — Use a local MapLibre geometry layer and partitioned runtime contract

- **Status:** Accepted runtime/geometry foundation on 2026-09-13; the no-basemap presentation constraint is superseded by D-024's attributed contextual-style fallback
- **Decision:** Use MapLibre GL JS with a minimal local style, the verified ECan Ashburton River GeoJSON polygon, and real WGS84 station coordinates. Deliver the application data through runtime contract `2.0.0`: a shell containing metadata, geometry, coverage, summaries, trends, and checksummed parameter-partitioned observation files decoded through typed lookup-backed rows. Lazy-load observation detail by parameter; do not add a basemap, remote tiles, API, database, binary format, or service worker for this local milestone.
- **Evidence:** The prior map was a CSS coordinate schematic and `mapShellOptions` returned an empty MapLibre style. The audited boundary response contains one verified Polygon (`CatchmentGroup=688`, source object `267`) and the site audit identifies 19 in-bound stations. The original formatted runtime asset was 10,473,514 bytes; the new shell is approximately 1.26 MB raw and the shell plus initial E. coli partition is approximately 1.41 MB raw, with repeated observation metadata represented through lookup indexes. The browser suite verified the local geometry, 19 station markers, coordinated selection, and asset timing.
- **Alternatives considered:** Keep the schematic; use a third-party basemap or remote tiles; retain the monolithic observation JSON; introduce a backend/database or binary browser format. These were rejected for this milestone because the schematic did not satisfy the accepted geography architecture, remote basemap terms are unresolved, the monolith delayed initial data delivery, and no measured need justified added runtime infrastructure.
- **Consequences:** The local geometry remains the authoritative fallback and runtime detail is staged without weakening normalized/audit assets or export provenance. D-024 adds optional contextual geography without changing the local data contract. The runtime shell/partitions, raw responses, and generated observations remain ignored; public observation delivery, freshness, attribution implementation, and basemap operations remain release gates.

## D-020 — Keep public observation release blocked after source-term review

- **Status:** Superseded by D-021 after the exact dataset-specific water-quality terms were retrieved on 2026-09-14
- **Decision:** Continue local processing and local dashboard review. Do not publicly host observation assets, enable public observation display, or distribute filtered observation CSV until ECan confirms that the exact legacy Hilltop observation route and the exact ArcGIS catchment layer permit those uses under applicable terms.
- **Evidence:** The official ECan Open Data agreement provides attribution, no-branding, no-advertising, freshness/removal, and general CC BY 4.0 language unless specifically stated otherwise. The ECan Developer Data Portal terms provide the exact attribution sentence but also prohibit licensing, sublicensing, or reselling API content. The inspected primary sources do not state whether those terms govern this legacy Hilltop route or grant public redistribution of its normalized/derived outputs or the exact ArcGIS polygon.
- **Alternatives considered:** Treat public endpoint accessibility or general CC BY language as sufficient; publish only code/UI without observation assets; request written route-specific confirmation; replace the source with an explicitly compatible dataset. The first is not defensible. The second is a possible portfolio fallback. The third is recommended. The fourth requires a new source and coverage decision.
- **Consequences at the time:** The application included official attribution and source links, retained retrieval/build metadata, and had a local-safe `tools/check_release_readiness.py` gate. D-021 replaces this blanket block with an evidence-backed CC BY release path and operational freshness safeguards.

## D-021 — Use the dataset-specific ECan water-quality CC BY route for public readiness

- **Status:** Accepted by implementation evidence on 2026-09-14; supersedes D-020's permanent release block
- **Decision:** Treat the current ECan water-quality Terms of Use, linked from the official water-quality page and retrieved from the official Trim document route, as the release basis for the same published Hilltop observations. Prepare public visualization, hosted normalized/derived assets, and filtered CSV export under CC BY 4.0 with explicit attribution, an accompanying Terms of Use link/copy, source provenance, current-or-remove operation, no ECan/Metro branding, and no advertising. Keep raw responses ignored and do not deploy in this milestone.
- **Evidence:** The exact one-page PDF (retrieved 2026-09-14; SHA-256 `87b0c0408c5e6dce82b0cf036acdcf8ffc78e7d31e45db2a1a60f15255ebe705`) states that the water-quality work is licensed for reuse under Creative Commons Attribution 4.0 International by Environment Canterbury and requires attribution and accompanying terms. The official page documents result download/print and the Hilltop service returned the same representative SQ35873 / Total Nitrogen result as the indexed official details page. ArcGIS item metadata separately records CC BY 3.0 New Zealand for the surface-water site and Major Catchment Boundary layers.
- **Alternatives considered:** Continue treating all public reuse as blocked; migrate to an undocumented internal website export; replace ECan data with another source; or contact ECan for bespoke permission. The first is no longer proportionate to the verified dataset-specific licence, the second is not a documented bulk route, the third is unnecessary, and the fourth is not required for the evidenced CC BY path.
- **Consequences:** Generated assets remain ignored and deployment remains a later operational action. The release checker can pass `--require-public-release` only for a fresh asset carrying `public_cc_by_attribution_freshness`, a matching terms hash, application attribution, and no tracked raw/generated outputs. The exact evidence and remaining freshness/removal obligations are in `docs/release/` and `docs/RELEASE_READINESS.md`.

## D-022 — Advance the analytical window after complete identified-source refresh

- **Status:** Accepted by implementation evidence on 2026-09-14
- **Decision:** Retain the available 2007–2025 history, use 2016–2025 as the primary dashboard window, and use 2020–2025 as the recent window. Keep the six core parameters and two secondary parameters unchanged.
- **Evidence:** The fresh all-19-site profile returned 11,230 observations through 2025-12-18. Each core parameter family had 2025 observations at 9–15 sites and represented all twelve calendar months. Lower-frequency site schedules and one metadata-supported no-row site remain explicit in the coverage audit.
- **Consequences:** The analytical version is `ashburton-analytical-v4-published-unflagged-2016-2025`. The window change does not change the quality policy, censoring safeguards, summary rules, or conservative trend method. The product must continue to describe sampled coverage rather than current catchment conditions.

## D-023 — Require exact station IDs for complete identified-source reconciliation

- **Status:** Accepted implementation decision on 2026-09-14
- **Decision:** Reconcile the official surface inventory and coordinate-bearing Hilltop catalog by exact stable station ID. Use coordinates for boundary inclusion and discrepancy diagnostics only; do not accept nearest-coordinate fallbacks when an exact ID is absent or ambiguous.
- **Evidence:** Count-checked ArcGIS pagination retrieved 6,266 surface features; 340 are inside/on the verified boundary. Hilltop returned 550 coordinate-bearing sites; 19 are inside/on the boundary and all 19 have exact surface-layer IDs. The maximum exact-ID coordinate discrepancy is approximately 0.0000153 degrees. There are 321 in-bound surface-only features and no Hilltop-only matches.
- **Consequences:** The application maps the 19 exact crosswalked Hilltop stations, while the compact audit retains unmatched spatial inventory and observation-free scope. This establishes complete reconciliation against identified inventories, not complete environmental monitoring coverage.

## D-024 — Adopt a map-centric viewport-filling dashboard shell

- **Status:** Accepted implementation decision on 2026-09-14
- **Decision:** Replace the document-style dashboard flow with a viewport-filling, map-centric MapLibre shell: compact header, control/context rail, dominant catchment map with selected-site history, and compact analytical rail. Keep full observation inspection and technical provenance in accessible secondary surfaces. Add availability, relative selected-window median, and neutral supported-trend map-display modes without introducing health categories or changing analytical results.
- **Evidence:** The owner-approved reference mockup establishes the intended information density and three-region hierarchy. The implemented shell passes 17 Playwright/axe browser checks, fits without body scrolling at 1440×900, 1536×864, and 1920×1080, and preserves the verified 19-site boundary and station workflow at compact/mobile sizes.
- **Basemap decision:** Attempt the no-key OpenFreeMap Positron style, with on-map OpenFreeMap/OpenMapTiles/OpenStreetMap attribution. Start from the verified local boundary/station layers and fail closed to that local geometry when the remote style or vector-source lifecycle is unavailable or incomplete. No paid provider, API key, or standard OSM raster endpoint is introduced.
- **Alternatives considered:** Retain the long analytical document flow; use an uncontextualized coordinate plot; add an unreviewed paid or key-requiring basemap; or silently continue with an incomplete remote style. The first three do not meet the approved product direction or release constraints; the last would misrepresent map readiness.
- **Consequences:** The map is the principal analytical surface while observed history, distributions, coverage, and transparent detail remain prominent. OpenFreeMap is an external operational dependency for contextual geography, but local geometry and station selection remain usable without it. Provider availability, attribution, and fallback behavior are now release/browser gates.
