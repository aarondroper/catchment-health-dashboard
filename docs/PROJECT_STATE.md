...
**As of:** 2026-09-13
**Authoritative status:** Ashburton–Hakatere `published_unflagged` analytical policy, deterministic local asset regeneration, and production-shaped local React integration verified; public source-term release and complete observation coverage remain open
...
The local production-shaped workflow is implemented against ignored generated assets. It is not a public production dashboard: the application has no remote tile dependency, uses a coordinate-based local map representation rather than verified catchment geometry, and falls back to a synthetic fixture if the local asset has not been prepared. The profile's site membership is spatially validated against the configured ECan major-catchment polygon, while observation coverage remains bounded to the retrieved public service history.
