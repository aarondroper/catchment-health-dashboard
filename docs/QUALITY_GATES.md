...
- `python3 tools/check_release_readiness.py` to verify local asset freshness/build metadata, official attribution presence, and that raw/generated observation outputs are not tracked. The command intentionally reports `PUBLIC_RELEASE=blocked` while `sourceTermsStatus` remains `local_processing_only_release_gate`; `--require-public-release` is an explicit failing gate until source terms are cleared.
