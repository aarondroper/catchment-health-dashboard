...
For the local production-shaped dashboard, run `cd web && npm ci && npm run typecheck && npm run test:unit && npm run build && npx playwright install chromium && npm run test:browser`. The browser suite starts Vite against the local prepared runtime shell and parameter partition, asserts that real assets and geometry are loaded instead of silently using the fixture, checks coordinated interactions, MapLibre selection and map-display modes, OpenFreeMap attribution and fail-closed local fallback, filtered export, keyboard access, and key empty/censored/indeterminate states, runs axe-core checks, measures asset timing, verifies responsive history SVG and plotting-region dimensions, and captures ignored screenshots at 1440×900, 1536×864, 1920×1080, 1024×768, and 390×844. Desktop body overflow is asserted separately at the same desktop sizes. Chromium is installed in the normal user cache; browser binaries, reports, traces, screenshots, and test results must not be committed.
