...
- `web/` contains a Vite/React/TypeScript dashboard shell, a map-centric three-region workspace, a MapLibre map with the audited ECan catchment polygon and station coordinates, optional OpenFreeMap Positron context with a timed local-geometry fallback, coordinated parameter/window/station/map-display state, typed inline-SVG and tabular analytical views, a responsive SVG history plot with a live `ResizeObserver` measurement lifecycle, viewport-safe body tooltips, and observation-dialog detail access, UTF-8 filtered CSV export through an accessible scope dialog, responsive/accessibility styling, expandable provenance notes with official attribution/source links, and a local ignored application-asset loader. The shared visual system uses locally bundled Source Sans 3 weights, restrained tokenized surfaces and controls, grouped analytical-rail panels, and no routine eyebrow-plus-heading duplication. MapLibre's built-in attribution is disabled at construction; one mode-aware, collapsed-by-default circular `i` control opens a viewport-positioned source overlay carrying the remote-provider or local-fallback credits, while context state remains non-visual live status. The control glyphs are replaceable inline SVGs and the left-rail geographic context is a small static Natural Earth-derived New Zealand locator, not a second map runtime. The loader reads the versioned runtime shell and parameter partitions from `web/public/data/ashburton/` when the developer has prepared them and falls back to a checked-in synthetic fixture with an explicit warning. `web/package-lock.json` and the Playwright/Vitest configurations provide reproducible frontend verification.
...
| Mapping | MapLibre GL JS with local GeoJSON boundary and station layers | Real ECan polygon, WGS84 stations, fit-to-bounds, selected/available states, map selection, keyboard-equivalent station control, OpenFreeMap Positron context, one accessible compact `i` attribution control with viewport overlay, and local fallback implemented |
...
- **External map context:** OpenFreeMap Positron is a browser-time, no-key contextual style. The dashboard does not send monitoring data to it; it shows the verified local boundary and station layers first, attributes OpenFreeMap/OpenMapTiles/OpenStreetMap through one accessible compact `i` disclosure with a viewport-positioned overlay when the remote context is loaded, and switches that disclosure to ECan-only spatial credits when the local geometry fallback is active.
