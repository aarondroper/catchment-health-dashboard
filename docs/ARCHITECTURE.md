...
| Mapping | MapLibre GL JS with local GeoJSON boundary and station layers | Real ECan polygon, WGS84 stations, fit-to-bounds, selected/available states, map selection, keyboard-equivalent station control, OpenFreeMap Positron context, one accessible compact attribution disclosure, and local fallback implemented |
...
- **External map context:** OpenFreeMap Positron is a browser-time, no-key contextual style. The dashboard does not send monitoring data to it; it shows the verified local boundary and station layers first, attributes OpenFreeMap/OpenMapTiles/OpenStreetMap through one accessible compact disclosure when the remote context is loaded, and switches that disclosure to ECan-only spatial credits when the local geometry fallback is active.
