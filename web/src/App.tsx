...
export function App() {
  ...
  return (
    <main className="app-shell">
      ...
      <details className="data-notes" data-testid="data-notes"><summary><span><span className="eyebrow">Transparency</span><strong>Data notes and provenance</strong></span><span className="summary-hint">Quality, methods, source, and release context</span></summary><div className="notes-grid"><div><h3>How to read this view</h3><p><strong>Published observation; no source quality code supplied</strong> describes returned observations without a quality field. They are usable for this exploratory local dashboard but are not explicitly verified, QC600, or “good.” The processing disposition is <code>published_unflagged</code>. Quality-coded, censored, excluded, and missing states remain distinguishable in the table and export.</p><p>Selected records: {Object.entries(qualityCounts).map(([key, value]) => `${key.replaceAll("_", " ")} (${value})`).join(" · ") || "none"}.</p></div><div><h3>Source and method</h3><p>Environment Canterbury (ECan) public water-quality service · 2007–2024 sampled history · current window {windowLabel(window)}. Medians, interquartile ranges, and neutral trends appear only where the documented coverage and censoring rules support them. The history is not continuous monitoring and does not represent unmonitored parts of the catchment.</p><p>This work uses data sourced from Environment Canterbury. <a href="https://www.ecan.govt.nz/data/water-quality-data" target="_blank" rel="noreferrer">Water-quality data source</a> · <a href="https://data.ecan.govt.nz/Catalogue/Agreement?AgreementFile=Agreement.htm&amp;AgreementRequirements=General" target="_blank" rel="noreferrer">data-use terms</a>.</p><p>Local build {asset.buildId ?? "development fixture"} · analytical version {asset.analyticalVersion} · source retrieval {asset.sourceRetrievedAt ?? "development fixture"}.</p></div><div><h3>Release context</h3><p>Assets are being processed locally for review. ECan attribution, freshness responsibility, and public observation redistribution remain release gates. No ECan branding, regulatory compliance claim, causal explanation, threshold, or composite score is presented.</p></div></div></details>
    </main>
  );
}
