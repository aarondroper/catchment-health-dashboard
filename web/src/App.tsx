  ...
  return (
    <main className="app-shell">
      ...
      <section className="insight-grid" aria-label="Selected site evidence">
        ...
        <section className="panel insight-panel" aria-labelledby="trend-title"><div className="panel-heading"><div><p className="eyebrow">Change over time</p><h2 id="trend-title">Trend evidence</h2></div><span className={`status-chip status-${trend?.status ?? "unavailable"}`}>{statusLabel(trend?.status)}</span></div>{trend?.status === "reported" ? <><p className="headline-value trend-value">{trend.direction}</p><p className="panel-intro">Neutral direction from a Theil–Sen estimate across {trend.calendarYearCount} calendar years.</p><p className="supporting-stat">This is not an improvement, deterioration, causal, or compliance claim.</p></> : <p className="state-copy">No trend estimate is shown because {reasonLabel(trend?.indeterminateReason ?? null)}. Censored values are not substituted.</p>}</section>
      ...
      <section className="panel comparison-panel" aria-labelledby="comparison-title"><div className="panel-heading"><div><p className="eyebrow">Across monitoring sites</p><h2 id="comparison-title">Reported median values</h2></div><span className="unit-label">{parameter.unit ?? "unit pending"}</span></div>{comparison.length > 1 ? <div className="comparison-table-wrap" tabIndex={0} role="region" aria-label="Cross-site reported medians"><table className="observation-table comparison-table"><caption>Sites with supported summaries in {windowLabel(window)}; sites without supported summaries remain represented on the map.</caption><thead><tr><th scope="col">Monitoring site</th><th scope="col">Median</th><th scope="col">Middle half</th><th scope="col">Eligible records</th></tr></thead><tbody>{comparison.map((row) => <tr key={row.stationId}><td>{stationLabel(asset.stations.find((item) => item.stationId === row.stationId))}</td><td>{displayNumber(row.value, row.unit)}</td><td>{displayNumber(row.q1, row.unit)}–{displayNumber(row.q3, row.unit)}</td><td>{row.eligibleNumericCount}</td></tr>)}</tbody></table></div> : <p className="state-copy">Only one or no monitoring sites has a supported summary for this parameter and period. The comparison is not presented as a catchment-wide result.</p>}</section>
